...
warnings.filterwarnings("once", category=NumbaPerformanceWarning)
...
def compute_accels(positions, masses, args):
    ...
    d_accels = cuda.to_device(accels.astype(np.float32))
    d_pairs = cuda.to_device(pairs)
    
    threadsperblock = 128
    blockspergrid = (len(pairs) + threadsperblock - 1) // threadsperblock
    
    compute_all_pair_forces[blockspergrid, threadsperblock](d_pairs,d_positions,d_masses,d_accels,epsilon,sigma,r_cap)
    accels = d_accels.copy_to_host()
